...
def _resolve_lang_value(selected_label: str, i18n: dict[str, str]) -> str:
    mapping = {
        i18n.get("ocr_lang_ch", ""): "ch",
        i18n.get("ocr_lang_en", ""): "en",
    }
    return mapping.get(selected_label, "ch")
...
TRANSLATIONS = {
    "en": {
        ...
        "ocr_lang_ch": "Chinese / Mixed Chinese-English",
        "ocr_lang_en": "English",
        "ocr_advanced_show": "Advanced OCR Options ▸",
        "ocr_advanced_hide": "Advanced OCR Options ▾",
        "ocr_det_db_thresh_label": "OCR Det Thresh (higher = stricter, default 0.35, range 0.1–0.9):",
        "ocr_det_db_box_thresh_label": "OCR Box Thresh (default 0.8, range 0.3–0.9):",
        "ocr_det_db_unclip_ratio_label": "OCR Unclip Ratio (lower = tighter, default Lite 0.9 / Server 1.0, range 0.5–2.0):",
    ...
    "zh": {
        ...
        "ocr_model_variant_lite": "Lite",
        "ocr_model_variant_server": "Server",
        "ocr_lang_label": "OCR 语言：",
        "ocr_lang_ch": "中文 / 中英混合",
        "ocr_lang_en": "英文",
        "ocr_advanced_show": "OCR 高级选项 ▸",
        "ocr_advanced_hide": "OCR 高级选项 ▾",
        "ocr_det_db_thresh_label": "OCR 检测阈值（越高越严格，默认 0.35，范围 0.1–0.9）：",
        "ocr_det_db_box_thresh_label": "OCR 框置信度阈值（默认 0.8，范围 0.3–0.9）：",
        "ocr_det_db_unclip_ratio_label": "OCR 框扩张比例（越小越紧，默认 Lite 0.9 / Server 1.0，范围 0.5–2.0）：",
        "start_button": "开始转换",
...
class AddTaskDialog(tk.Toplevel):
    def __init__(self, parent):
        ...
        self.ocr_lang = tk.StringVar(value=self.i18n["ocr_lang_ch"])
    ...
    def _create_widgets(self):
        ...
        tk.Label(frame, text=self.i18n['ocr_lang_label']).grid(row=7, column=0, sticky="w", pady=5)
        lang_options = (
            self.i18n['ocr_lang_ch'],
            self.i18n['ocr_lang_en'],
        )
        tk.OptionMenu(frame, self.ocr_lang, *lang_options).grid(row=7, column=1, sticky="ew", padx=5)
        ...
        advanced_toggle.grid(row=8, column=0, columnspan=2, sticky="w", pady=5)

        self.ocr_advanced_frame = tk.Frame(frame)
        self.ocr_advanced_frame.grid(row=9, column=0, columnspan=3, sticky="ew")
        ...
        options_frame.grid(row=10, column=0, columnspan=3, pady=10, sticky="w")
        ...
        buttons_frame.grid(row=11, column=0, columnspan=3, pady=5)
    ...
    def _on_ok(self):
        ...
        self.result = {
            ...
            "ocr_lang": _resolve_lang_value(self.ocr_lang.get(), self.i18n),
...
class App(tk.Tk):
    ...
    def _create_widgets(self):
        ...
        tk.Label(self.single_mode_frame, text=self.i18n['ocr_lang_label']).grid(row=7, column=0, sticky="w", pady=2)
        lang_options = (
            self.i18n['ocr_lang_ch'],
            self.i18n['ocr_lang_en'],
        )
        ...
        ).grid(row=7, column=1, sticky="ew", padx=5)
        ...
        single_advanced_toggle.grid(row=8, column=0, columnspan=2, sticky="w", pady=2)

        self.single_ocr_advanced_frame = tk.Frame(self.single_mode_frame)
        self.single_ocr_advanced_frame.grid(row=9, column=0, columnspan=3, sticky="ew")
    ...
    def _on_lang_change(self, _selected_label):
        if self.shared_ocr_engine is not None:
            self.shared_ocr_engine.lang = _resolve_lang_value(self.ocr_lang.get(), self.i18n)
    ...
    def _run_single_conversion(self, json_path, input_path, output_path):
        ...
        resolved_lang = _resolve_lang_value(self.ocr_lang.get(), self.i18n)
        ...
        if self.shared_ocr_engine is None:
            ...
            self.shared_ocr_engine.lang = resolved_lang
    ...
    def _run_batch_conversion(self):
        ...
        for i, task in enumerate(self.task_list):
            ...
            try:
                ...
                resolved_lang = task.get('ocr_lang') or "ch"
